...
name: latex-document
description: >
  Universal LaTeX skill: create, compile, convert any document to PDF with PNG previews. Supports: resumes/CVs, cover letters, reports, invoices, academic papers, theses, Beamer presentations, scientific posters, formal letters, exams/quizzes, books, cheat sheets, formula sheets, fillable PDF forms, conditional content, mail merge (CSV/JSON), version diffing, charts (pgfplots/matplotlib), tables (booktabs/CSV), TikZ images, Mermaid/Graphviz/PlantUML diagrams, AI images, watermarks, bibliography (BibTeX/biblatex), CJK/multi-language, algorithms, tcolorbox, siunitx, Pandoc conversion (Markdown/DOCX/HTML↔LaTeX), PDF-to-LaTeX OCR. Compile script: pdflatex/xelatex/lualatex, latexmk, texfot, PDF/A, verbosity control. Also: encrypt/password-protect, merge/split/rotate/crop PDFs, optimize for web, lint/word-count/stats, fetch BibTeX from DOI, debug errors, extract text/tables (pdfplumber), OCR scans (pytesseract), programmatic PDF (reportlab), add watermarks, extract metadata. Use for any LaTeX authoring, PDF manipulation, format conversion, OCR, or document generation task.
---
